...
def mock_data():
    timestamps = pd.date_range(
        start="2023-01-01", end="2023-01-20", freq="10min"
    )
...
def perfect_data():
    ...
    )  # Initialize scores as a Series

    # Assign scores based on defined time ranges
    for time_range in time_ranges:
        mask = (timestamps >= time_range["start"]) & (
            timestamps <= time_range["end"]
        )
        scores[mask] = np.random.uniform(
            time_range["low"], time_range["high"], mask.sum()
        )
...
def test_prepare_dataframe(mock_events, mock_data):
    ...
    assert len(data) == len(
        timestamps
    ), "Data length should match timestamps length."

    # Assert training data
    assert len(train_data) > 0, "Train data should not be empty."
    assert np.allclose(
        data[data["event"] == "healthy_train"]["score"].values, train_data
...
def test_compute_tr_by_events(mock_events, mock_data):
    ...
    assert expected_events.issubset(
        result.keys()
    ), "Result keys should match the events."

    # Assert all values are floats
    assert all(
        isinstance(v, float) for v in result.values()
    ), "All values in the result should be floats."
...
def test_compute_tr_by_events_perfect_detector(mock_events, perfect_data):
    ...
    assert result["damage_1"] == pytest.approx(
        1.0, rel=1e-6
    ), "TPR for damage_1 should be 1.0."
    assert result["healthy_train"] == pytest.approx(
        0.02, abs=0.01
    ), "TPR for healthy_train should be 0.01"
...
def test_compute_mean_variation_perfect_detector(mock_events, perfect_data):
    ...
    assert (
        result["damage_1"] > result["healthy_train"]
    ), "Mean variation for damage_1 should be higher than healthy_train."
    assert (
        result["damage_1"] > result["healthy_2"]
    ), "Mean variation for damage_1 should be higher than healthy_2."
